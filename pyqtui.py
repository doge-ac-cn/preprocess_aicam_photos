...
class win(QDialog):
        ...
        with open(self.data_path + '/Result/msg.json') as fd:
            with open("test.json", "r") as data_file:
                ...
                self.data_dict['calibInfo']['VideoChannels'][0]['MediaInfo']['FrameNum'] = len(
                    os.listdir(self.data_path)) - 1  # FrameNumprint(os.listdir(self.data_path)[-1])
                self.data_dict['calibInfo']['VideoChannels'][0]['MediaInfo']['breakFrameNum'] = \
                os.listdir(self.data_path)[-1]  # breakFrameNum
    ...
    def loadImage(self):
        ...
        #cv2.imread如果读不到会返回None,不会直接报错，不能直接try catch ，所以根据返回值判断
        ...
        for target in targets['targets']:
            ...
            if target['obj']['type'] == 1:
                img = cv2.putText(img, "pig" + str(target['obj']['confidence']), (x_center, y_center), cv2.FONT_ITALIC,
                                  0.8, (255, 255, 255), 2)
            else:
                img = cv2.putText(img, "people" + str(target['obj']['confidence']), (x_center, y_center),
